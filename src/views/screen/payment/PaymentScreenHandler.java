			...
			if(newValue.contains("http://127.0.0.1:50387/?")) {
                ...
                if(response.getVnp_ResponseCode().equals("00")){
					try {
						...
						//System.out.println("Invoice [" +invoice.getId() +", " + invoice.getStatus()+ ", "+ invoice.getAmount()+ ", " +invoice.getOrderId() + ", " + invoice.getPaypalId());
						this.invoice.updateInvoice(invoice.getId(), invoice.getStatus(), invoice.getPaypalId(), invoice.getAmount());
						//PlaceOrderController placeOrderController
						System.out.println("Invoice [ ID = " +invoice.getId() +", status = " + invoice.getStatus()+ ", amount = "+ invoice.getAmount()+ ", orderID = " +invoice.getOrderId() + ", VNPayID = " + invoice.getPaypalId()+" ]");
